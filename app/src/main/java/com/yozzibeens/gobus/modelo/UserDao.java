...
public class UserDao extends AbstractDao<User, Long> {
    ...
    protected void bindValues(SQLiteStatement stmt, User entity) {
        ...
        Long Cabbie_Id = entity.getUser_Id();
        if (Cabbie_Id != null) {
            stmt.bindLong(1, Cabbie_Id);
        }
    ...
    public User readEntity(Cursor cursor, int offset) {
        User entity = new User( //
                cursor.isNull(offset + 1) ? null : cursor.getLong(offset + 0), // User_Id
                cursor.isNull(offset + 2) ? null : cursor.getString(offset + 1), // Name
                cursor.isNull(offset + 3) ? null : cursor.getString(offset + 2) // Email
        );
        return entity;
    }
